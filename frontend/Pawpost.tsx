...
import './pawpost.css';
import { normalizePreferences, canDeliverNotice, wantsNotice, wantsDesktopNotice, activityEvents, channelDefaults, type Preferences, type ChannelMode } from './notification-policy.mjs';

type PawEvent = { id: number; at: string; kind: string; channel: string; sender: string; world: string; text: string; outgoing: boolean; attention: boolean; conversation: string | null; suppressAlert?: boolean };
type Watcher = { name: string; world: string; distance: number };
type RecentWatcher = { name: string; world: string; lastSeen: string };
type Snapshot = { session: string; cursor: number; events: PawEvent[]; watchers: Watcher[]; recentWatchers?: RecentWatcher[]; player: { online: boolean; name: string; world: string; zone: string; targetTracking: boolean; gameFocused?: boolean } };
...
export default function Pawpost() {
  ...
  function alertFor(incoming: PawEvent[], gameFocused: boolean | undefined) {
    const prefs = preferencesRef.current;
    const eligible = incoming.filter(e => canDeliverNotice(e, prefs, gameFocused, document.hasFocus() && !document.hidden));
  ...
  function accept(snapshot: Snapshot) {
    ...
    if (initialized.current) {
      alertFor(fresh, snapshot.player.gameFocused);
